package com.lionel.utils.QRCode.demo1;
...
public class QRCodeUtil {    
        ...
        private static BufferedImage createImage(String content,BufferedImage logoImage, String bottomDes,  
            ...
            BufferedImage image = new BufferedImage(width+100, tempHeight+100,    
                    BufferedImage.TYPE_INT_RGB);   
            /*for (int x = 50; x < width+50; x++) {    
                for (int y = 50; y < height+50; y++) {    
                    image.setRGB(x, y, bitMatrix.get(x-50, y-50) ? 0xFF000000    
                            : 0xFFFFFFFF);//fff白色。000黑色    
                }    
            } */
            for (int x = 0; x < width+100; x++) {    
                for (int y = 0; y < height+100; y++) {
                	if(x<=50 || x>=650 || y>=650 || y<=50)
        ...
        private static void addFontImage(BufferedImage source, String declareText) {  
            ...
            Image src =textImage;    
            graph.drawImage(src, 50, 650, width, height, null);     
            graph.dispose();    
        }  
        ...
        public static void main(String[] args) throws Exception {    
            ...
            //QRCodeUtil.encodeOfPath(text, "e:/test/01.jpg",null,"e:/test", true);    
            //QRCodeUtil.encode(text,"e:/test"); // 不带logo   
            BufferedImage bi = ImageIO.read(new File("e:/WPS/01.jpg"));  
               QRCodeUtil.encode(text,bi,"www.baidu.com","123","e:/test", true);  
        } 
}
